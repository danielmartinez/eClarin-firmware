...
uint8_t mocoMode = 1;	/* 0: Serial , 1: MIDI */
uint8_t highSpeed = 0;		/* 0: normal speed(31250bps),
...
static uint8_t utxrdy = FALSE;	/* USB Packet ready in utx_buf */
static uint8_t rx_buf[RX_SIZE];	/* tempory buffer */
static uint8_t utx_buf[RX_SIZE];	/* BULK_IN buffer */
...
USB_ClassInfo_CDC_Device_t VirtualSerial_CDC_Interface = {
  .Config = {
    ...
    .DataINEndpoint = {
      .Address          = (ENDPOINT_DIR_IN | CDC_TX_EPNUM),
      .Size             = CDC_TXRX_EPSIZE,
      .Banks            = 1,
    },
    .DataOUTEndpoint = {
      .Address          = (ENDPOINT_DIR_OUT | CDC_RX_EPNUM),
      .Size             = CDC_TXRX_EPSIZE,
      .Banks            = 1,
    },
    .NotificationEndpoint = {
      .Address          = (ENDPOINT_DIR_IN | CDC_NOTIFICATION_EPNUM),
...
void parseUSBMidiMessage(uint8_t *data, uint8_t len) {
  uint8_t cin = (*data) & 0x0f;	/* ignore cable no */
  uint8_t i;
...
uint8_t parseSerialMidiMessage(uint8_t RxByte) {
  static uint8_t PC = 0;
  static uint8_t SysEx = FALSE;
  static uint8_t sysExCnt = 0;
  static uint8_t stateTransTable[] = {
    0, 				/* 0 dummy */
    0,				/* 1 dummy */
    3,				/* 2->3 NOTE OFF (3) */
    2 | 0x80,			/* 3->2 */
...
#define CNTMAX 40
  ...
  for (;;){
    ...
    while (MIDI_Device_ReceiveEventPacket(&Keyboard_MIDI_Interface, &ReceivedMIDIEvent)) {
      ...
      parseUSBMidiMessage((uint8_t *)&ReceivedMIDIEvent, 4);
